...
VALUES (
    ...
    'Development CoC',
    'dev-coc',
    -- Round 5 §16.E (revised): dev-coc has features.reentryMode=true so the
    -- existing reentry-* Playwright specs (which auth via @dev.fabt.org users)
    -- continue to surface reentry filters/dialog/eligibility. The "gate works"
...
-- ============================================================================
-- PHASE M-LIGHT: BLUE RIDGE CoC (demo) — second tenant for multi-tenant demo
-- ============================================================================
-- Fictional region (not a HUD-registered CoC). "Blue Ridge" is a multi-state
-- mountain-range geographic feature — no real-jurisdiction CoC exists by this
-- name. Per docs-repo PR #3 + D15 design decision: PLATFORM_ADMIN is tenant-
-- scoped today (not platform-spanning); per-tenant seeds are intentional.
-- Phase F task 7.12 + role split (issue #141) will introduce the true
-- platform-spanning identity later.
--
-- UUID conventions for the three demo-tenant shape:
--   dev-coc         tenant a0000000-...-000001   users b0000000-...   shelters d0000000-...
--   dev-coc-west    tenant a0000000-...-000002   users b0000001-...   shelters d0000001-...
--   dev-coc-east    tenant a0000000-...-000003   users b0000002-...   shelters d0000002-...
-- ============================================================================
...
-- Blue Ridge shelters — 3 demo rows. Street addresses remain fake ("Fictional
-- Way") per D12, but cities use real western-NC mountain towns (Boone,
...
-- ============================================================================
-- PHASE M-LIGHT: PAMLICO SOUND CoC (demo) — third tenant for multi-tenant demo
-- ============================================================================
-- Fictional region (not a HUD-registered CoC). "Pamlico Sound" is an NC
-- coastal lagoon — geographic feature, not a jurisdiction. Same tenant-scoped
-- PLATFORM_ADMIN posture as Blue Ridge. See header above for UUID convention.
-- ============================================================================
...
-- Pamlico Sound shelters — 3 demo rows. Street addresses remain fake
...
-- ============================================================================
-- PHASE M-LIGHT: bed_availability for the 6 new demo shelters
-- Moderate occupancy so search surfaces have real results + enough open beds
-- for a Darius-style walkthrough. Mirrors dev-coc utilization profile (~70%).
-- ============================================================================
INSERT INTO bed_availability (shelter_id, tenant_id, population_type, beds_total, beds_occupied, beds_on_hold, accepting_new_guests, snapshot_ts, updated_by, notes) VALUES
-- Blue Ridge — Example House North (family, 20 total, 13 occupied = 7 available)
...
-- Blue Ridge — Blue Ridge Example Shelter (single adult, 40 total, 28 occupied = 12 available)
...
-- Blue Ridge — Safe Haven Demo DV West (3 population splits; mirrors dev-coc DV pattern)
...
-- Pamlico — Example Coastal House (family, 25 total, 17 occupied = 8 available)
...
-- Pamlico — Pamlico Example Shelter (single adult, 50 total, 38 occupied = 12 available)
...
-- Pamlico — Safe Haven Demo DV East (DV + women)
