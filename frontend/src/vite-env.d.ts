...
interface ImportMetaEnv {
   ...
   *
   * - `'true'` (default in dev + prod): the platform chunk is dynamically
   *   imported on demand; routes resolve normally.
   * - `'false'`: the lazy import literal is dead-code-eliminated by
   *   Rollup (a top-level `if (... !== 'true')` branch is the eliminator),
   *   `dist/assets/platform-*.js` is absent from the build, and
   *   `/platform/*` routes fall through to the standard NotFound.
   *
   ...
   */
  readonly VITE_PLATFORM_UI_ENABLED: string;
}
