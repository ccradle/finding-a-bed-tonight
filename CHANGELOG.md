...
- **New env var `FABT_PLATFORM_CONTACT_EMAIL`** — read by Spring at startup via `application.yml` (`fabt.platform.contact-email: ${FABT_PLATFORM_CONTACT_EMAIL:}`), backed by `PlatformContactProperties` (`@ConfigurationProperties(prefix = "fabt.platform")`, no `@Email` / `@Validated` — startup does NOT fail on a malformed value). Empty/unset = "not configured" → public endpoint returns `platform.email: null` and the static-site footers fall back to the GitHub-Issues link (intentional degraded-but-safe state). The literal value is operator-managed in `~/fabt-secrets/.env.prod` and never appears in this repo (anti-bot-scrape posture: see CI guard below).
- **`PATCH /api/v1/admin/tenants/{tenantId}/contact-email`** — COC_ADMIN-scoped via `@PreAuthorize`, tenant-scoped via in-method check against the JWT tenant claim (cross-tenant attempts return **403** and emit a defense-in-depth audit row). Accepts `{ email: <RFC-5322-shape, ≤254 chars> | null | "" }` — both `null` and empty-string clear the per-tenant override and let the platform value take effect. Persists to `tenant.config.contact.email`. Emits a `TENANT_CONFIG_UPDATED` audit row capturing old + new email values for forensic trace (values are NOT masked at audit-write time; `audit_events` carries them as plaintext under JSONB `details`). No new `AuditEventType` enum value — reuses the existing v0.55 entry per the JSONB-key convention. **DV-policy gated**: returns `400` with `errorCode: tenant.contactEmail.dvPolicyForbidden` if the tenant has `dv_policy_enabled=true` (clearing always allowed, even DV-on). The DV-policy gate is enforced server-side; the admin UI mirrors it as a UX cue (input + Save button disabled). `DemoGuardFilter` (active under the `demo` profile) blocks all PATCH writes on `/admin/**`, so on the prod (`lite,demo,observability`) container this endpoint is operator-only via SSH-tunnel platform login — same posture as the v0.56 dv-policy and observability writes.
- **`GET /api/v1/public/contact-info`** — anonymous OR authenticated. Bucket4j rate limit 60 req/min keyed on `X-Real-IP` with fallback to `getRemoteAddr()` when the header is missing or blank (capacity / refill / cache-key spec in `application.yml`; Caffeine cache backing in `application.conf`). Response shape: authed → `{ platform: { email }, tenant: { slug, email } }`; unauthed → `{ platform: { email }, tenant: null }`. **Cache-Control split by auth state** — unauthed responses ship `Cache-Control: public, max-age=3600` + `ETag` (Cloudflare may cache); authed responses ship `Cache-Control: private, max-age=3600` + `Vary: Authorization` + `ETag` (browsers cache, shared caches must NOT). The split was added after spec-review caught a gap in an earlier draft that would have allowed Cloudflare's edge to serve one tenant's authenticated body to a different caller. Authed responses also enforce the v0.56 DV-policy read-side suppression — `tenant.email` returns `null` when the calling tenant has `dv_policy_enabled=true`.
- **New ArchUnit allowlist guard** — `PublicEndpointAllowlistTest` enumerates every controller method mapped under `/api/v1/public/**` and fails the build if a new one ships without explicit allowlist + reviewer sign-off. Closes the design-review concern that the public endpoint surface needs a hard gate against future expansion drift.
- **Structured error code registry expanded.** `org.fabt.shared.errors.ErrorCodes` (introduced in v0.56) gains `tenant.contactEmail.dvPolicyForbidden` and the per-field bean-validation codes the PATCH endpoint surfaces via `context.detail` (RFC-5322 shape rejection, ≤254-char rejection). Surfaced via `context.errorCode` / `context.detail` in 400 response bodies for client-parseable UX (frontend `parseContactEmailError` keys off these).
- **Frontend — login-page footer + admin Settings panel.** New `ContactInfoProvider` + `useContactInfo` hook hydrate the React app from the public endpoint at boot; the `LoginPage` footer renders the platform email (or GH-Issues fallback). New `ContactSettings.tsx` panel is the first card in the admin Settings tab — modal-confirm Save flow, RFC-5322 client-side shape validation, structured-error parsing via the extracted `parseContactEmailError` helper. Input + Save button are disabled when the tenant has `dv_policy_enabled=true` (mirroring the backend gate). The bundled `ApiError.context` parsing fix from v0.56 carries forward — the structured `tenant.contactEmail.dvPolicyForbidden` rejection surfaces as a localized inline message rather than a generic 400 toast.
- **Static-site contact-info hydration (docs repo).** New `/contact.js` asset deployed to `/var/www/findabed-docs/contact.js` (source lives in the docs repo, NOT this code repo) fetches `/api/v1/public/contact-info` and hydrates `<a class="contact-email" hidden>` placeholders to `mailto:` links. Lang-aware (EN/ES dictionary embedded; reads `document.documentElement.lang`). Falls back to the GH-Issues link with a `console.warn` on any fetch failure. **14 modified HTML pages** gain the canonical `<p class="footer-contact">` placeholder + `<noscript>` GH-fallback + `<script defer src="/contact.js"></script>` near `</body>`: `index.html`, `404.html`, `demo/index.html` + `dvindex.html` + `hmisindex.html` + `analyticsindex.html` + `reentry-story.html`, `demo/for-cities.html` + `for-coc-admins.html` + `for-coordinators.html` + `for-funders.html`, `demo/outreach-one-pager.html` + `pitch-briefs.html` + `shelter-onboarding.html`. **Audience-page CTA upgrades** ship in 5 of those pages (demo/for-cities + for-coc-admins + for-coordinators + for-funders + outreach-one-pager) — the existing CoC-admin email links and "contact us" prose now route through the same hydration path so the contact surface is unified.
- **CI guard `scripts/ci/check-contact-placeholder.sh` (docs repo).** Lives in the docs repo (NOT this code repo); runs on every push there. Five per-page assertions: (1) `class="contact-email"` placeholder present, (2) `aria-live="polite"` on the placeholder, (3) `<script defer src="/contact.js">` near `</body>`, (4) `<noscript>` block with GH-Issues fallback link, (5) **no literal `[A-Za-z0-9._%+-]+@findabed.org` anywhere in committed HTML/JS**. The fifth check is the anti-scrape primary defense (Cloudflare Email Address Obfuscation is the secondary; see `reference_cloudflare_email_obfuscation_dependency.md`). The guard is documented in the docs-repo README; a `--selftest` mode is on the v0.57.1 backlog.
- **Cloudflare posture unchanged.** Email Address Obfuscation remains `On` (defense-in-depth alongside the JS-injection primary). Bot Fight Mode unchanged. The new public endpoint flows through the existing zone — no new rules.
- **i18n.** 10 new EN keys covering the admin panel (input label + helper text + DV-policy-disabled tooltip + save success/error toasts) and the login-footer placeholder. ES counterparts are AI-synthetic (Claude with web-citation grounding, NOT a native speaker) per `feedback_truthfulness_above_all`; tracked at `reference_es_json_ai_synthetic_reviewed.md` for native-speaker review. Audience-page HTML CTAs are pure markup with English-only copy at this release; ES audience-page localization is a v0.57.1+ follow-up.
- **Tests.** Backend: 6 new test classes — `ContactEmailControllerTest` (12 cases incl. cross-tenant 403, DV-policy 400 with errorCode + count, RFC-5322 shape rejection, audit-row content), `ContactInfoControllerTest` (14 cases incl. unauthed-vs-authed cache headers, ETag round-trip, DV-suppression for authed callers on flag-on tenants), `ContactInfoControllerEmptyPlatformTest` (3 cases for the unset-env-var fallback), `PlatformContactPropertiesTest` (3 cases), `TenantContactEmailHelperTest` (11 cases for the read-side helper on `Tenant.readContactEmail(...)`), `PublicEndpointAllowlistTest` (ArchUnit). Frontend: new `ContactInfoContext.test.ts` (provider + `deriveContactInfoState` helper) and `ContactSettings.test.ts` (8 cases on `parseContactEmailError`). Playwright: new `e2e/playwright/tests/contact-info-static.spec.ts` covering JS-disabled (`<noscript>` fallback rendering) + lang-aware (`document.documentElement.lang` switching the dict) on root + for-cities + 404 pages.
- **Save-path coverage gap (disclosed).** `scripts/deploy-rehearsal.sh` does not exercise the contact-email PATCH end-to-end — the rehearsal stack lacks a tenant-JWT acquisition step, and `DemoGuardFilter` blocks the PATCH on the prod (`demo`-profile) container so a real-prod-call validation isn't possible from the rehearsal harness either. Save-path is unit-tested + integration-tested in the backend suite; a v0.57.1 follow-up extends the rehearsal to PATCH a throwaway contact-email + assert the audit row.
- **Deploy notes:** see `docs/oracle-update-notes-v0.57.0.md` (this repo) — ground-truthed runbook with the new env-var injection step, the static-content scp + Cloudflare Purge sequence, the empty-string sentinel for unset env, and the all-3-demo-tenants-are-DV-policy-on caveat (authed `/contact-info` curls return `tenant.email: null` post-deploy on every demo tenant; this is read-side suppression by design, not a bug).
