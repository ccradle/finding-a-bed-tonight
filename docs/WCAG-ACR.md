...
| **2.1.1 Keyboard** | Partially Supports | All primary workflows are keyboard-operable. Admin tab bar supports arrow key navigation per W3C APG tabs pattern. Skip-to-content link available. Modal dialogs (session timeout, change password, admin reset password) implement focus management with Escape to close. Full keyboard audit of all interactive elements pending. |
