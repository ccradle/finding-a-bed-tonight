...
### In-app Issue Reporting + Feedback Links (v0.57.4, planned)

Three new feedback surfaces consume the existing `useContactInfo()` hook from info-email-contact: an authenticated-app footer "Report a Problem" link (every page of `Layout.tsx`), a mobile kebab "Help" item (between Security and Sign Out), and a landing-page "Feedback & Support" section (`index.html`) with three GitHub paths (Report a Problem, Request a Feature, Ask a Question → Discussions Q&A). New `ReportProblemLink.tsx` exports the URL-builder helpers (`buildReportProblemUrl`, `buildIssueChooserUrl`, `buildIssuesIndexUrl`, `shouldRouteToMailto`) and the `<FooterReportProblemLink>` component.

**URL construction is allowlist-constrained** (warroom B1 + H1) — pre-fill values are compile-time constants (`template`, `labels=triage`), or version-derived (`fabt_version` from `/api/v1/version`, null-guarded so a pre-resolution click omits the param). The builder SHALL NOT read from `window.location`, `document.title`, route parameters, JWT claim values, user role/email/ID, tenant slug, or any user input — preventing label-injection via hostile share-links and accidentally funneling PII into a world-readable GitHub issue body. CI guard `scripts/ci/check-feedback-link-discipline.sh` enforces both the URL-builder discipline AND that any new `target="_blank"` link in `frontend/src/**/*.tsx` carries `rel="noopener noreferrer"`.

**DV-policy authenticated-surface gate (warroom B3 Casey veto + R2-B1)** — when the authenticated user's tenant has `dv_policy_enabled === true` AND a mailto fallback is available, the footer link + kebab Help item replace the GitHub URL with `mailto:{resolvedEmail}`. Sourced from `useContactInfo().tenant?.dvPolicyEnabled`, NOT from the JWT (the JWT decode doesn't carry the flag and `GET /api/v1/tenants/{id}/config` is `@PreAuthorize("hasRole('COC_ADMIN')")` — OUTREACH and COORDINATOR roles cannot read it elsewhere). The DV gate exists because survivors borrowing a coordinator's screen — or the coordinator herself — must not have the path-of-least-resistance be "type PII into a world-readable issue body." The landing-page surface is unauthenticated and continues to expose the GitHub paths (the gate is render-time only, not URL-borne).

**Small backend delta** — `ContactInfoController.buildResponseBody` surfaces an existing-evaluated boolean (`tenant.dvPolicyEnabled`) on the public `GET /api/v1/public/contact-info` response. No new endpoint, no new DB read (reuses `Tenant.isDvPolicyEnabled` already called for read-side email suppression), no new Flyway migration. Anonymous + platform-operator sessions continue to receive no `tenant` block (anon-leak regression test guards this).

**JS-disabled fallback** lives in `frontend/index.html` outside `#root` so it survives React mounting — a static GitHub-Issues link surfaces when JavaScript is disabled. Three rounds of multi-persona warroom (Marcus, Casey, Sam, Riley, Alex, Jordan, Devon) shaped the design before code; spec carryover MEDIUMs and NITs are documented in `openspec/changes/issue-reporting-feedback/tasks.md` as non-blocking polish.

### Platform + Per-Tenant Contact Email (v0.57, active)

The platform contact email is now public via `GET /api/v1/public/contact-info` (anonymous OR authenticated, Bucket4j rate-limited 60r/m/IP, cache-headers split by auth state — `public` for unauthed, `private` + `Vary: Authorization` for authed, both with ETags). Backed by `FABT_PLATFORM_CONTACT_EMAIL` env var read at startup via `PlatformContactProperties`. CoC admins set a per-tenant override via `PATCH /api/v1/admin/tenants/{id}/contact-email` (mirrors the hold-duration pattern: `@PreAuthorize("hasRole('COC_ADMIN')")` + tenant-scope guard + `TENANT_CONFIG_UPDATED` audit emission + DV-policy gate — non-empty writes return 400 `tenant.contactEmail.dvPolicyForbidden` when the tenant has `dv_policy_enabled=true`; empty-clear always allowed). The admin surface is `ContactSettings.tsx` in the `/admin` Settings tab.

**Read-side DV-policy suppression** belt-and-suspenders the write-side gate: authed `/contact-info` responses return `tenant.email=null` when the calling tenant has DV-policy on, even if a stale value persists in JSONB (e.g. a write that happened before the flag was enabled). **ArchUnit `PublicEndpointAllowlistTest`** enumerates classes permitted to declare `/api/v1/public/**` mappings; new public endpoints require an explicit allowlist update + reviewer sign-off.

**Static-site contact-info hydration** lives in the docs repo: `/contact.js` fetches the public endpoint and hydrates `<a class="contact-email" hidden>` placeholders to `mailto:` links on 14 in-scope HTML pages. Lang-aware (EN/ES dict embedded, reads `document.documentElement.lang`); falls back to a `<noscript>` GitHub-Issues link on any fetch failure. CI guard `scripts/ci/check-contact-placeholder.sh` (also docs repo) asserts 5 per-page invariants and a no-literal-email regex (primary anti-scrape defense; Cloudflare Email Address Obfuscation is the secondary). Forward-compat: the `useContactInfo()` hook is the single subscription point for the upcoming in-app issue-reporting work (GH #67).

**Shipped:** v0.57.0 2026-05-05 (aborted/recovered; pom + env-passthrough hotfix in v0.57.1, documented in `CHANGELOG.md [v0.57.1]` § "Recovery sequence executed" rather than a separate runbook). v0.57.1 + v0.57.2 are backend-only patches over the same Flyway HWM (V98 — no new migration; `contact.email` reuses the existing `tenant.config` JSONB column under a new sub-key). Cache-control split (B1, caught pre-ship in spec review) prevents Cloudflare edge from serving one tenant's authenticated body to a different caller. Deploy specifics: [`oracle-update-notes-v0.57.0.md`](./oracle-update-notes-v0.57.0.md) (info-email-contact ship) + [`oracle-update-notes-v0.57.2.md`](./oracle-update-notes-v0.57.2.md) (SYSTEM_TENANT + SSE + rehearsal hardening patch).

### DV-Policy Tenant Flag + Platform-Observability Split (v0.56, active)

Two openspec changes shipped together. The **DV-policy** flag (`tenant.config.dv_policy_enabled`, V97 backfill) acknowledges at the tenant level that a CoC operates Domestic Violence shelters, and a `ShelterService` invariant rejects per-shelter `dv_shelter=true` writes when the flag is off. Disabling the flag is forbidden while active DV shelters exist. The dedicated `PATCH /api/v1/admin/tenants/{id}/dv-policy` endpoint is COC_ADMIN-scoped and requires `dvAccess=true` on the JWT (RLS coupling per design D10); cross-tenant probes return 403 with no body and emit a `tenant.crossTenantAccess` audit row.

The **platform-observability split** decouples 6 tenant-agnostic config fields (Prometheus on/off, OTel tracing on/off + endpoint, three monitor cadences) from the per-tenant config they were incorrectly stored in. New `platform_config` singleton table (V98) houses them; `GET/PUT /api/v1/platform/observability` (PLATFORM_OPERATOR + `@PlatformAdminOnly` + `X-Platform-Justification` header) flips them on the Platform Operator Dashboard via 6 inline-edit ObservabilityActionCard surfaces. The one tenant-specific field (the Fahrenheit threshold for surge activation) moved to `/admin#surge` with a new `PUT /api/v1/tenants/{id}/surge-threshold` COC_ADMIN endpoint. `OperationalMonitorService` rewired from literal `@Scheduled` rates to a `SchedulingConfigurer` + `TaskScheduler` + dynamic `ScheduledFuture` pattern (mirrors `BatchJobScheduler`), so platform-operator interval changes take effect on the next reschedule cycle without a backend restart.

**Accessibility:** `PlatformActionCard` switched from native `disabled` to `aria-disabled="true"` + click guard per the [W3C ARIA APG keyboard-interface guidance](https://www.w3.org/WAI/ARIA/apg/practices/keyboard-interface/#focusabilityofdisabledcontrols) — flag-gated buttons now stay focusable so a keyboard-only operator can land on them and discover the disabled-state tooltip via screen reader. **BREAKING (operator-facing):** the `/admin#observability` tab is gone; CoC admins lose write access to the 6 platform-wide fields (which they never reliably had post-G-4.4 — every save 400'd). 5 new structured error codes registered in `ErrorCodes.java` (`platform.observability.intervalOutOfRange`, `tracingEndpointMalformed`, `fieldTypeMismatch`, `unknownField`, `tenant.surgeThreshold.outOfRange`). Per-field audit emission via `PLATFORM_OBSERVABILITY_UPDATED` `AuditEventType`. Bundled bug fix: `ApiError` class now reads `context` (matching backend) instead of `details` — silently dropped structured error payloads for ALL 400s pre-fix.

V97-V98 are additive + forward-compatible — service rollback (revert the JAR) is fully safe; old code reads from per-tenant config which still exists (design D3 backward-read; v0.58+ migration drops the obsoleted JSONB keys). For deploy specifics + post-deploy smoke contract, see [`oracle-update-notes-v0.56.0.md`](./oracle-update-notes-v0.56.0.md). For the dv-policy onboarding sequence on fresh CoCs (operator workflow + audit-first-then-throw ordering on rejected disables), see `openspec/changes/dv-policy-tenant-flag/runbook-fragments.md` in the docs repo.

### Reentry Mode (v0.55, active)

Per-tenant feature flag `tenant.config.features.reentryMode` (boolean, default `false`) gates the reentry-aware UI surfaces (outreach search advanced filters, admin shelter-edit `EligibilityCriteriaSection`, hold-with-attribution `HoldDialog`, coordinator-dashboard inline client-name display) and the API serialization of three `ReservationResponse` PII fields (`heldForClientName`, `heldForClientDob`, `holdNotes`). The serialization gate at `ReservationResponse.from(Reservation, String, String)` (lines 80-92) keys on `TenantContext.getReentryMode()` and is the primary control; the frontend gating sites are defense-in-depth UX, not access control. Flag flip is read at JWT-issuance time and emitted as a `reentryMode` claim, so propagation is bounded by the 15-minute access-token TTL (`JwtService.reentryModeCache` adds a 60s upper bound on stale-cache reads).

V91-V96 deliver the schema: `shelter.shelter_type` + `county` + `requires_verification_call`, `shelter_constraints.eligibility_criteria` JSONB + GIN index, `reservation.held_for_client_*_encrypted` columns + `tenant_dek.purpose='RESERVATION_PII'`, plus V95+V96 reentry-shelter seed inserts (Onslow Womens, Beaufort Reentry Annex, Henderson Reentry House). All migrations are additive + forward-compatible with v0.54 backend (rollback path is image-only; schema stays).

**For enable/disable mechanics** (CoC-admin curl + PLATFORM_OPERATOR SSH-tunnel pattern, both directions, audit-event contract, local-dev quick-flip via direct DB UPDATE, V95+V96 manual replay after `--fresh`), see [`operations/reentry-mode-user-guide.md`](operations/reentry-mode-user-guide.md). **For design rationale** (D1 three-way `acceptsFelonies` evaluator, D3 no-DB-enum for counties, D4 two-layer PII posture, D6 disclaimer co-rendering CI guard, D13 frontend gate posture), see `openspec/changes/archive/2026-05-01-transitional-reentry-support/design.md` in the docs repo (post-archive). Audit-event types: `RESERVATION_HELD_FOR_CLIENT_RECORDED`, `RESERVATION_PII_DECRYPTED_ON_READ` (Caffeine-throttled to one row per coordinator/shelter/hour), `RESERVATION_PII_PURGED`. Verification curl + Flyway HWM expected output: `oracle-update-notes-v0.55.0.md` §6.
...
`TenantScopedCacheService` (`@Service` in `org.fabt.shared.cache`) wraps the existing `CacheService` with four load-bearing contracts: key prefix with `|` separator sourced from `TenantContext`; value stamp-and-verify via `TenantScopedValue<T>(UUID tenantId, T value)` envelope; `invalidateTenant(UUID)` backed by `CacheService.evictAllByPrefix`; Micrometer observability (`fabt.cache.{get,put}{cache,tenant,result}`). `DetachedAuditPersister` sibling uses `PROPAGATION_REQUIRES_NEW` so `CROSS_TENANT_CACHE_READ` audit rows survive attacker-triggered caller rollback (Marcus lens, design-c D-C-9).

**v0.46 (preparatory) shipped the idle bean.** v0.47 activates it across all 9 application-layer cache call sites: `AnalyticsService` (6 methods: `getUtilization`, `getDemand`, `getCapacity`, `getDvSummary`, `getGeographic`, `getHmisHealth`), `BedSearchService.doSearch`, `AvailabilityService.createSnapshot`, and `ShelterService.evictTenantShelterCaches`. Composite-key callers strip the caller-side `tenantId` prefix (wrapper re-prefixes); tenant-singleton callers collapse to the `"latest"` literal per D-4.b-2. The `FamilyCArchitectureTest.PENDING_MIGRATION_SITES` allowlist is drained to `Set.of()` — that empty state is the v0.47.0 release gate per spec requirement `pending-migration-sites-drained`. The `fabt.cache.registered_cache_names` gauge stays at 11.0 (`CacheNames` inventory unchanged); `CacheIsolationDiscoveryTest` parametrizes over all 11 constants with exact `.hasSize(11)` pin so future additions fire a build-time reminder to update per-cache tests.

**Observability in v0.47.** Four Prometheus alert rules shipped in `deploy/prometheus/phase-c-cache-isolation.rules.yml`: `FabtPhaseCCrossTenantCacheRead` (CRITICAL, immediate page), `FabtPhaseCMalformedCacheEntry` (WARN 15m), `FabtPhaseCCacheHitRateCollapse` (WARN 30m, dormant until T+7d of scrape history), `FabtPhaseCDetachedAuditPersistFailure` (WARN 15m — silent audit-write loss signal parallel to Phase B's `fabt.audit.rls_rejected.count`). All four filter on `env="prod"` so CI / dev scrapes can't false-page. v0.47 also rewires `docker-compose.yml` + `prometheus.yml` to actually load rule files (base compose Prometheus service gains `--web.enable-lifecycle` + rules volume mount; `rule_files:` glob added — retroactively loads Phase B's 5 rules which had been on disk since v0.43.1 but never read). Alert triage: `docs/security/phase-c-cache-isolation-runbook.md`.

Exception messages propagated from the wrapper to HTTP responses carry short action tags only (`CROSS_TENANT_CACHE_READ`, `MALFORMED_CACHE_ENTRY`, `TENANT_CONTEXT_UNBOUND`) — never UUIDs or payload fragments (OWASP ASVS 5.0 §7.4.1). Pinned at `TenantScopedCacheServiceUnitTest:355-371`.

Cross-references: `docs/architecture/redis-pooling-adr.md` (three-shape deployment taxonomy, HIPAA AES-256/TLS-1.3 scope for silo tier), `openspec/changes/multi-tenant-production-readiness/design-c-cache-isolation.md` (D-C-1..13 + D-4.b-1..7 warroom decisions), `docs/oracle-update-notes-v0.47.0.md` (deploy runbook).
