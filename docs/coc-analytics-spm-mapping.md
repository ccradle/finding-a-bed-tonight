...
| **HIC export** (bed inventory CSV) | SPM 3 (input data) | **Direct input** | HIC CSV matches HUD Inventory.csv schema (FY2024+): 17 columns including InventoryID, CoCCode, HouseholdType (integer codes 1/3/4), veteran bed breakdown, ESBedType, InventoryStartDate. DV shelters aggregated with small-cell suppression (≥3 shelters). |
| **PIT export** (sheltered count CSV) | SPM 3 (input data) | **Working document** | Sheltered PIT count by household type with HUD integer codes. Note: PIT data is submitted via direct entry in HDX 2.0, not CSV upload. This CSV supports CoC admin preparation. |
