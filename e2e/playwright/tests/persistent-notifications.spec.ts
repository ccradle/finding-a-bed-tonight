 ...
 *
 * Seed data (T-63) provides:
 * - cocadmin: referral.requested (ACTION_REQUIRED), surge.activated (CRITICAL)
...
test.describe('Persistent Notifications', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!loginResp.ok) throw new Error('beforeAll: cocadmin login failed — is the stack running?');
    ...
    if (unread < 2) {
      throw new Error(
        `beforeAll: cocadmin has ${unread} unread notifications (need >= 2). ` +
  ...
  test('T-54: coordinator sees pending referral banner', async ({ coordinatorPage }) => {
    await coordinatorPage.goto('/coordinator');
    await expect(coordinatorPage.locator('[data-testid="coordinator-heading"]')).toBeVisible();

    const banner = coordinatorPage.locator('[data-testid="coordinator-referral-banner"]');
  ...
  test('T-56: bell badge shows unread count > 0 on login', async ({ coordinatorPage }) => {
    await coordinatorPage.goto('/coordinator');
    await expect(coordinatorPage.locator('[data-testid="coordinator-heading"]')).toBeVisible();

    const bellButton = coordinatorPage.locator('[data-testid="notification-bell-button"]');
  ...
  test('T-58: WCAG banners have role="alert"', async ({ coordinatorPage }) => {
    await coordinatorPage.goto('/coordinator');
    await expect(coordinatorPage.locator('[data-testid="coordinator-heading"]')).toBeVisible();

    // Referral banner — if present
    const refBanner = coordinatorPage.locator('[data-testid="coordinator-referral-banner"]');
    ...
    const critBanner = coordinatorPage.locator('[data-testid="critical-notification-banner"]');
  ...
  test('T-58b: CRITICAL banner visible when CRITICAL notifications exist', async ({ coordinatorPage }) => {
    await coordinatorPage.goto('/coordinator');
    await expect(coordinatorPage.locator('[data-testid="coordinator-heading"]')).toBeVisible();
  ...
  test('T-58e: notification dropdown renders i18n text, not raw type', async ({ coordinatorPage }) => {
    await coordinatorPage.goto('/coordinator');
    await expect(coordinatorPage.locator('[data-testid="coordinator-heading"]')).toBeVisible();
    ...
    const panel = coordinatorPage.locator('[data-testid="notification-panel"]');
    await expect(panel).toBeVisible();
  ...
  test('T-58c: mark all as read preserves CRITICAL badge count', async ({ coordinatorPage }) => {
    await coordinatorPage.goto('/coordinator');
    await expect(coordinatorPage.locator('[data-testid="coordinator-heading"]')).toBeVisible();
    ...
    const markAllBtn = coordinatorPage.locator('[data-testid="mark-all-read-button"]');
  ...
  test('T-57: mark notification as read decrements badge', async ({ coordinatorPage }) => {
